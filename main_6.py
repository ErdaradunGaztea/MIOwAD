...
nn0 = NeuralNet(1, normal_init)\
    .add_layer(Layer(10, sigmoid))\
    .add_layer(Layer(5, sigmoid))\
    .add_layer(Layer(1, linear))
nn0.budget.set_epoch_limit(50).set_detection_limit(1.5)
...
nn0.train(np.transpose(x_train), np.transpose(y_train), learning_rate=0.005, batch_size=8)
...
# plot
plt.plot(np.arange(len(nn0.loss_history)), nn0.loss_history, label="none")
